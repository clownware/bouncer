...
function noulProbability(answer) {
  if (answer === void 0 || answer.type !== "noul") return void 0;
  return typeof answer.noul === "number" && Number.isFinite(answer.noul) ? answer.noul : void 0;
}

// src/adapters/jev.ts
var DEFAULT_BASE_URL = "https://api.typesafe.ai/v1/systemone";
...
function asAdapterError(err, timeoutMs) {
...
function parseAnswer(value) {
  ...
  switch (record2["type"]) {
    case "noul": {
      const noul = record2["noul"];
      if (typeof noul !== "number" || !Number.isFinite(noul) || noul < 0 || noul > 1) return void 0;
      return { type: "noul", noul };
    }
    case "choice": {
      const choice = record2["choice"];
      const probabilities = record2["probabilities"];
      if (typeof choice !== "string" || !isNumberRecord(probabilities)) return void 0;
      return {
        type: "choice",
        choice,
        probabilities,
...
function isNumberRecord(value) {
  return typeof value === "object" && value !== null && !Array.isArray(value) && Object.values(value).every((v) => typeof v === "number");
}
function backoffFor(attempt, response) {
  const header = response?.headers?.get?.("retry-after");
  if (header !== void 0 && header !== null) {
    const seconds = Number(header);
    if (Number.isFinite(seconds) && seconds >= 0) return Math.min(seconds * 1e3, 1e3);
  }
...
var MockAdapter = class {
  ...
  scoreFor(name, state) {
    ...
    if (heuristic === void 0) return 0.05;
...
function describeComparison(comparison) {
  switch (comparison.kind) {
    ...
    case "lt":
      return `<${comparison.value}`;
    case "range":
      return `${comparison.low}..${comparison.high}`;
  }
}
...
function escalationFor(set, decision, answers, item) {
  if (decision.reason.kind !== "rule") return void 0;
  if (decision.verdict === "allow") return void 0;
  return {
    item,
...
function signalsFor(set, answers, decidedBy) {
  const signals = [];
  let decidedTaken = false;
  for (const rule of set.rules) {
    if (rule.condition === void 0) continue;
...
function manifestOf(items, itemsJudged) {
  return {
    itemsJudged,
    items,
...
function buildState(input) {
  ...
  const clean = (value) => {
    const { text: text2, kinds: found } = redact(value);
...
function describeAction(input, clean) {
  const { toolName, toolInput, cwd } = input;
  switch (toolName) {
    case "Bash": {
      const action = { kind: "run_shell_command" };
      const command = toolInput["command"];
    ...
    case "Write": {
      ...
      return {
        ...
        bytes: typeof content === "string" ? Buffer.byteLength(content, "utf8") : void 0,
    ...
    case "Edit": {
      ...
      return {
        ...
        bytes_removed: typeof oldString === "string" ? Buffer.byteLength(oldString, "utf8") : void 0,
        bytes_added: typeof newString === "string" ? Buffer.byteLength(newString, "utf8") : void 0
      };
    }
    case "NotebookEdit": {
      ...
      return {
        ...
        ...path,
        cell: typeof cellId === "string" ? cellId : void 0,
        edit_mode: typeof toolInput["edit_mode"] === "string" ? toolInput["edit_mode"] : void 0
      };
    }
    default: {
...
function pathFacts(raw, cwd, clean) {
  if (typeof raw !== "string" || raw.length === 0) {
    return { path: void 0, inside_project: void 0 };
  }
  if (cwd === void 0 || cwd.length === 0) {
    return { path: clean((0, import_node_path.basename)(raw)), inside_project: void 0 };
  }
  const absolute = (0, import_node_path.isAbsolute)(raw) ? (0, import_node_path.resolve)(raw) : (0, import_node_path.resolve)(cwd, raw);
  const rel = (0, import_node_path.relative)((0, import_node_path.resolve)(cwd), absolute);
  const inside = rel.length > 0 && !rel.startsWith("..") && !(0, import_node_path.isAbsolute)(rel);
  const facts = {
    ...
    path: clean(inside ? rel : (0, import_node_path.basename)(absolute)),
...
function describeSensitivity(path) {
  const parts = path.split(import_node_path.sep).filter((p) => p.length > 0);
...
function describeOutside(absolute) {
...
function truncateStrings(record2, limit) {
  return Object.fromEntries(
    Object.entries(record2).map(
      ([key, value]) => typeof value === "string" && value.length > limit ? [key, `${value.slice(0, limit)}\u2026 [truncated]`] : [key, value]
    )
  );
}
...
function writeAtomic(file, text) {
  ...
  try {
    ...
    (0, import_node_fs.writeFileSync)(temp, text, "utf8");
    (0, import_node_fs.renameSync)(temp, file);
    return true;
  } catch {
    try {
      (0, import_node_fs.unlinkSync)(temp);
    } catch {
    }
    return false;
  }
...
function readAll(dir) {
  let parsed;
  try {
    parsed = JSON.parse((0, import_node_fs2.readFileSync)((0, import_node_path3.join)(dir, BREAKER_FILE), "utf8"));
  } catch {
    return [];
  }
...
function write(dir, state) {
  ...
  writeAtomic((0, import_node_path3.join)(dir, BREAKER_FILE), JSON.stringify({ sessions }));
}
function isState(value) {
  return typeof value === "object" && value !== null && typeof value["session_id"] === "string";
}
...
function record(state, outcome) {
  ...
  const next = { ...state, warmed: true, consecutive_failures: failures, consecutive_slow: slow };
  if (failures >= FAILURE_LIMIT) {
    return {
      state: { ...next, tripped: { reason: "failures", at: (/* @__PURE__ */ new Date()).toISOString() } },
  ...
  if (slow >= SLOW_LIMIT) {
    return {
      state: { ...next, tripped: { reason: "latency", at: (/* @__PURE__ */ new Date()).toISOString() } },
...
function fileFor(dir, path) {
  ...
  return (0, import_node_path4.join)(dir, DIR, `${hash.toString(16).padStart(8, "0")}.json`);
}
...
function policyCandidates(cwd, pluginRoot2) {
  ...
  if (override !== void 0 && override.length > 0) candidates.push((0, import_node_path5.resolve)(override));
  const repoRoot = findRepoRoot(cwd);
  if (repoRoot !== void 0) {
    candidates.push((0, import_node_path5.join)(repoRoot, ".bouncer.yaml"), (0, import_node_path5.join)(repoRoot, ".bouncer.yml"));
  }
  candidates.push((0, import_node_path5.join)((0, import_node_os.homedir)(), ".bouncer", "bouncer.yaml"));
  if (pluginRoot2 !== void 0) candidates.push((0, import_node_path5.join)(pluginRoot2, "policy", "default.yaml"));
  return candidates;
}
function findRepoRoot(from) {
  let current = (0, import_node_path5.resolve)(from);
  for (; ; ) {
    if ((0, import_node_fs4.existsSync)((0, import_node_path5.join)(current, ".git"))) return current;
...
function pluginRoot() {
  ...
  const candidate = (0, import_node_path5.resolve)((0, import_node_path5.dirname)(script), "..");
  return (0, import_node_fs4.existsSync)((0, import_node_path5.join)(candidate, "policy", "default.yaml")) ? candidate : void 0;
}
function apiKey() {
...
function append(dir, record2, name = LOG_FILE) {
  try {
    ...
    (0, import_node_fs5.appendFileSync)(file, `${JSON.stringify(safe)}
`, "utf8");
  } catch {
  }
}
...
function rotateIfOversized(file) {
  let size;
  try {
    size = (0, import_node_fs5.statSync)(file).size;
  } catch {
    return;
  }
  if (size < MAX_LOG_BYTES) return;
  (0, import_node_fs5.renameSync)(file, `${file}.1`);
}
...
function tail(dir, count, name = LOG_FILE) {
  let raw;
  try {
    raw = (0, import_node_fs5.readFileSync)((0, import_node_path6.join)(dir, name), "utf8");
  } catch {
    return [];
  }
  const records = [];
...
// src/hooks/pretooluse.ts
async function runPreToolUse(payload, options = {}) {
  const now = options.now ?? Date.now;
  const started = now();
  const tool = typeof payload.tool_name === "string" ? payload.tool_name : "";
  if (tool.length === 0) return void 0;
  ...
  if (policy === void 0) {
    if (resolved.source === "(none)") return void 0;
    const state2 = read(dir, sessionId);
    if (!shouldNotify(state2, "policy-error")) return void 0;
    write(dir, markNotified(state2, "policy-error"));
  ...
  const breakerState = read(dir, sessionId);
  const status2 = check(breakerState);
  if (status2.tripped) {
    append(dir, {
      ...base,
      emitted: null,
  ...
  try {
    ...
    return standDown(dir, base, breakerState, status2, err, now() - started, policy);
  }
...
function standDown(dir, base, state, status2, err, totalMs, policy) {
  ...
  if (denies) {
    return {
      ...
      ...next.message !== void 0 ? { systemMessage: next.message } : {}
    };
  }
...
function targetExistsFor(tool, toolInput, options) {
  ...
  return exists === void 0 ? {} : { targetExists: exists };
}
function defaultTargetExists(path) {
  try {
    return (0, import_node_fs6.existsSync)(path);
  } catch {
    return void 0;
  }
}
...
`;
  ...
  lines.push(`Policy:  ${resolved.source}`);
  lines.push(`Log:     ${(0, import_node_path7.join)(dir, LOG_FILE)}`);
  ...
  const standingDown = readAll(dir).filter((s) => s.tripped !== void 0);
...
function summarize(records) {
  ...
  if (judged.length > 0) {
    const escalated = judged.map((r) => r.escalation).filter((e) => e !== void 0);
  ...
  const errors = records.filter((r) => r.error !== void 0);
  if (errors.length > 0) {
    const last = errors[0];
...
function explain2(toolUseId) {
  const records = tail(dataDir(), SEARCH_DEPTH);
  if (records.length === 0) {
    return "No decisions logged yet.\n";
  }
  const record2 = toolUseId === void 0 || toolUseId.length === 0 ? records.find((r) => r.reason.kind !== "fast-path") ?? records[0] : records.find((r) => r.tool_use_id === toolUseId);
  if (record2 === void 0) {
    return `No decision logged for ${toolUseId}. It may have scrolled out of the last ${SEARCH_DEPTH} entries.
`;
  }
  return render(record2);
}
function render(record2) {
  ...
  if (record2.agent_type !== void 0) {
    lines.push(`Called by subagent: ${record2.agent_type}`);
  }
  if (record2.answers !== void 0 && Object.keys(record2.answers).length > 0) {
    lines.push("", "Judgments:");
    const entries = Object.entries(record2.answers).sort((a, b) => b[1] - a[1]);
  ...
  if (record2.error !== void 0) {
    lines.push("", `Error: ${record2.error.kind} \u2014 ${record2.error.message}`);
  }
  if (record2.state !== void 0) {
    lines.push("", "What was sent to the classifier:", `  ${record2.state}`);
  }
  if (record2.redacted_kinds !== void 0 && record2.redacted_kinds.length > 0) {
    lines.push("", `Redacted before sending: ${record2.redacted_kinds.join(", ")}`);
  }
  const adapter = record2.latency_ms.adapter;
  lines.push("", `Latency: ${record2.latency_ms.total}ms total${adapter !== void 0 ? `, ${adapter}ms in the classifier` : ""}${record2.warmup === true ? "  (first call of the session)" : ""}`);
  return `${lines.join("\n")}
`;
}
function describe(record2) {
...
function loadFixtures(path) {
  return parseFixtures((0, import_node_fs7.readFileSync)(path, "utf8"));
}
...
`);
  ...
  const fixturePath = args.fixtures ?? (0, import_node_path8.join)(root, "fixtures", "gate.jsonl");
  let fixtures;
  try {
...
function writeAnswers(path, answered, policy, setName, backend) {
  ...
  (0, import_node_fs8.writeFileSync)(path, `${lines.map((l) => JSON.stringify(l)).join("\n")}
`, "utf8");
}
function fromLog(args, policy, fixtures, write3) {
  let source;
  try {
    source = (0, import_node_fs8.readFileSync)(args.from, "utf8");
  } catch (err) {
...
function loadItems(path) {
  const stats = (0, import_node_fs9.statSync)(path);
  if (stats.isDirectory()) return loadDirectory(path);
  const source = (0, import_node_fs9.readFileSync)(path, "utf8");
  return (0, import_node_path9.extname)(path).toLowerCase() === ".json" ? loadJsonArray(source, path) : loadJsonl(source, path);
}
...
function loadDirectory(root) {
  ...
  const walk = (dir, depth) => {
    if (depth > MAX_DEPTH2) {
      skipped.push({ path: (0, import_node_path9.relative)(root, dir) || ".", why: `nested more than ${MAX_DEPTH2} deep` });
      return;
    }
    for (const entry of (0, import_node_fs9.readdirSync)(dir, { withFileTypes: true }).sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      const id = (0, import_node_path9.relative)(root, full).split(import_node_path9.sep).join("/");
      let size;
      try {
        size = (0, import_node_fs9.statSync)(full).size;
      } catch (err) {
      ...
      let buffer;
      try {
        buffer = (0, import_node_fs9.readFileSync)(full);
      } catch (err) {
        skipped.push({ path: id, why: message(err) });
        continue;
      }
...
`
  ...
  let batch;
  try {
    batch = loadItems((0, import_node_path10.resolve)(args.path));
  } catch (err) {
...
function writeLog(path, run2, policy) {
  ...
  const ts = (/* @__PURE__ */ new Date()).toISOString();
  for (const item of run2.items) {
    append((0, import_node_path10.dirname)(path), recordFor(item, run2, policy, ts), basenameOf(path));
  }
}
...
function discoverSkills(cwd) {
  ...
  const userSkills = (0, import_node_path11.join)(home, ".claude", "skills");
  collect(userSkills, () => plainSkillDir(userSkills, "user"));
  for (const bucket of bucketsIn((0, import_node_path11.join)(home, ".claude", "skills", "synced"))) {
    const manifest = (0, import_node_path11.join)(bucket, "manifest.json");
    collect(manifest, () => syncedSkills(manifest));
  }
  for (const bucket of bucketsIn((0, import_node_path11.join)(home, ".claude", "plugins", "synced"))) {
    const manifest = (0, import_node_path11.join)(bucket, "manifest.json");
    collect(manifest, () => pluginSkills(manifest, bucket, (name) => (0, import_node_path11.join)(bucket, name, "skills")));
  }
  const installed = (0, import_node_path11.join)(home, ".claude", "plugins", "installed_plugins.json");
...
function plainSkillDir(dir, origin) {
  ...
  for (const name of directoriesIn(dir)) {
    ...
    const frontmatter = read3((0, import_node_path11.join)(dir, name, "SKILL.md"));
...
function installedPluginSkills(file) {
  ...
  for (const entry of entries) {
    ...
    const dir = (0, import_node_path11.join)(installPath, "skills");
    for (const skillName of directoriesIn(dir)) {
      const frontmatter = read3((0, import_node_path11.join)(dir, skillName, "SKILL.md"));
      if (frontmatter === void 0) continue;
...
function desktopManifests(home) {
  const root = (0, import_node_path11.join)(home, "Library", "Application Support", "Claude", "local-agent-mode-sessions");
  if (!(0, import_node_fs11.existsSync)(root)) return [];
  const found = [];
  let visited = 0;
  for (const outer of directoriesIn(root)) {
    for (const inner of directoriesIn((0, import_node_path11.join)(root, outer))) {
      if (++visited > MAX_SESSION_DIRS) return found;
      const session = (0, import_node_path11.join)(root, outer, inner);
      const rpm = (0, import_node_path11.join)(session, "rpm", "manifest.json");
      if ((0, import_node_fs11.existsSync)(rpm)) {
        found.push({
          path: rpm,
          gather: () => pluginSkills(rpm, session, (name) => (0, import_node_path11.join)(session, "rpm", `plugin_${name}`, "skills"))
        });
      }
      for (const bucket of nestedBuckets((0, import_node_path11.join)(session, "skills-plugin"))) {
        const manifest = (0, import_node_path11.join)(bucket, "manifest.json");
        if ((0, import_node_fs11.existsSync)(manifest)) found.push({ path: manifest, gather: () => syncedSkills(manifest) });
      }
    }
  }
  return found;
...
function bucketsIn(dir) {
  return directoriesIn(dir).map((name) => (0, import_node_path11.join)(dir, name));
}
function nestedBuckets(dir) {
  const buckets = [];
  for (const outer of directoriesIn(dir)) {
    for (const inner of directoriesIn((0, import_node_path11.join)(dir, outer))) {
      buckets.push((0, import_node_path11.join)(dir, outer, inner));
    }
  }
  return buckets;
}
function directoriesIn(dir) {
  try {
    return (0, import_node_fs11.readdirSync)(dir, { withFileTypes: true }).filter((entry) => entry.isDirectory() && !entry.name.startsWith(".")).map((entry) => entry.name);
  } catch {
    return [];
  }
}
function read3(file) {
  try {
    return (0, import_node_fs11.readFileSync)(file, "utf8");
  } catch {
    return void 0;
  }
}
...
function signatureOf(sources) {
  ...
  for (const source of sources) {
    try {
      const stat = (0, import_node_fs11.statSync)(source);
...
function projectSkillsDir(cwd) {
  ...
  return root === void 0 ? void 0 : (0, import_node_path11.join)(root, ".claude", "skills");
}
function findRepoRoot2(from) {
  let current = from;
  for (let depth = 0; depth < 32; depth++) {
    if ((0, import_node_fs11.existsSync)((0, import_node_path11.join)(current, ".git"))) return current;
    const parent = (0, import_node_path11.dirname)(current);
    if (parent === current) return void 0;
    current = parent;
  }
  return void 0;
