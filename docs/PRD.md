...
- **API key handling:** read from `BOUNCER_TYPESAFE_API_KEY` or `TYPESAFE_API_KEY` env only. Never stored by the plugin. Never logged. Resolving an `op://` reference at hook time was dropped: `op read` spawns a subprocess and can raise a biometric prompt, so it would put Touch ID in the middle of an agent run. This section used to add that 1Password support could return as a `SessionStart` hook resolving once per session; it cannot, checked 2026-09-19. `CLAUDE_ENV_FILE` is scoped to the Bash tool — the docs define it as a script run "before each Bash command in the same shell process" — so a `PreToolUse` hook never sources it (anthropics/claude-code#60697), and a plugin's `SessionStart` hook is handed it empty regardless (#11649). The key comes from the environment Claude Code was started with; `docs/dogfooding.md` §2 lists the three ways to set it.
